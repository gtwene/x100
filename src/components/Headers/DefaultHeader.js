...
export default function DefaultHeader({
  ...
  const AccountBalanceEnquiryModal = ({ name, showModal, setShowModal }) => {
      ...
      axios
        ...
        .then((response) => {
          ...
          if (response.length > 0) {
            ...
            setDescription("");
            setAccountStatus("");
            setCrAccruedInterest("");
            setDbAccruedInterest("");
            setDbAccruedPenalty("");
            setArrearsInterest("");
            setCrAccruedInterestRate("");
            setDbAccruedInterestRate("");
            setDbAccruedPenaltyRate("");
            setArrearsInterestRate("");
            setInterestIntSuspense("");
            setPenaltyIntSuspense("");
            setArrearsIntSuspense("");
            setDebitLimit("");
            setCreditLimit("");
            setUnclearedBalance("");
            setClearedBalance("");
      ...
      if (e.key === "Enter") {
        ...
        axios
          ...
          .then((response) => {
            ...
            let arrearsResponse = response.data.arrearsdata;
            let blockedResponse = response.data.blockeddata;
            let ledgerResponse = response.data.allLedger;
            // console.log(res, "fammm")

            setCurrency(results.rows[0][0]);
            setProduct(results.rows[0][1]);
            setBranch(results.rows[0][2]);
            setDescription(results.rows[0][3]);
            setAccountStatus(results.rows[0][4]);
            setCrAccruedInterest(results.rows[0][5]);
            setDbAccruedInterest(results.rows[0][6]);
            setDbAccruedPenalty(results.rows[0][7]);
            setArrearsInterest(results.rows[0][8]);
            setCrAccruedInterestRate(results.rows[0][9]);
            setDbAccruedInterestRate(results.rows[0][10]);
            setDbAccruedPenaltyRate(results.rows[0][11]);
            setArrearsInterestRate(results.rows[0][12]);
            setInterestIntSuspense(results.rows[0][13]);
            setPenaltyIntSuspense(results.rows[0][14]);
            setArrearsIntSuspense(results.rows[0][15]);
            setDebitLimit(results.rows[0][16]);
            setCreditLimit(results.rows[0][17]);
            setUnclearedBalance(results.rows[0][18]);
            setClearedBalance(results.rows[0][19]);

            setLedgerBalance(ledgerResponse.rows[0][0]);
            setAvailableBalance(ledgerResponse.rows[0][1]);

            setTotalArrears(arrearsResponse.rows[0][0]);
            setBlockedAmount(blockedResponse.rows[0][0]);

            // console.log(craccruedInterest, "oooo");
            // console.log(results, "yolooooo");
    ...
    const handleFetch = (e) => {
      // console.log(accountNumber, "accountNumber1111");
      // console.log(startDate, "start_date11111");
      // console.log(endDate, "end_date1111");
      ...
      // console.log(start_date, "eDate111");

      axios
        .post(API_SERVER + "/getTrans", {
          accountNumber: accountNumber,
        ...
        .then((response) => {
          ...
          for (let i = 0; i < results.length; i++) {
            ...
            const local_equivalent_cr = results[i].local_equivalent_cr;
            const local_equivalent_db = results[i].local_equivalent_db;
    ...
    useEffect(() => {
      const getDates = () => {
        return axios.get(API_SERVER + "/get-dates");
      };

      Promise.all([getDates()])
      .then((response) => {
        const dateResponse = response[0].data[0];

        const start_date = dateResponse.start_date;
        const end_date = dateResponse.end_date;

        // handleTrans(start_date , end_date)
                ...
                <div style={{ display: "flex" }}>
                  ...
                  <div style={{ flex: 0.96 }}>
                    ...
                    <div
                      style={{
                        display: "grid",
                        gridTemplateColumns: "2.15fr 0.86fr 0.99fr",
                        columnGap: "24px",
                        rowGap: "10px",
                        paddingTop: "10px",
                      }}
          ...
          <div className="pt-4 ml-12 mr-12">
              ...
              >
                <tbody>
                  <tr>
                    ...
                    <td>
                      <p
                      className="leftHeaderTextDefault"
                        style={{
                          color: "white",
                          marginTop: "20px",
                          marginLeft: "7px",
            ...
            <div className="float-right">
              <p
                ...
                className="rightHeaderTextDefault"
              >
                You're logged in as,
                <MDBIcon
                  style={{
            ...
            >
              ...
              >
                <Select
                  ...
                  } transition ease-in-out duration-100 transform translate-x-0 searchBoxDefaultC`}
                  onKeyDown={(e) => {
                    if (e.key === "Enter") {
