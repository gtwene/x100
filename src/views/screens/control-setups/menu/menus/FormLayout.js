...
export default function FormLayout({ formData, disabled, handleClose }) {
  ...
  async function handleSubmit(event) {
    ...
    if (menuLevel === "3" && document.getElementById("icon").files[0]) {
      ...
      if (
        ...
        try {
          const response = await axios.post(
            API_SERVER + "/update-menu-details",
            formData
          );
      ...
      const id = document.getElementById("id").value.replace(/'/g, "''");
      const menu_id = document.getElementById("menu_id").value.replace(/'/g, "''");
      const menu_level = document.getElementById("menu_level").value.replace(/'/g, "''");
      const menu_name = document.getElementById("menu_name").value.replace(/'/g, "''");
      const parent_menu_id = document.getElementById("parent_menu_id").value.replace(/'/g, "''");
      const menu_groupings = document.getElementById("menu_groupings").value.replace(/'/g, "''");
      const icon = document.getElementById("icon").value.replace(/'/g, "''");
      const color = document.getElementById("color").value.replace(/'/g, "''");
      const type_code = document.getElementById("type_code").value.replace(/'/g, "''");
      const file_name = document.getElementById("file_name").value.replace(/'/g, "''");
      const menu_url = document.getElementById("menu_url").value.replace(/'/g, "''");
      const menu_permitted = document.getElementById("menu_permitted").value.replace(/'/g, "''");
      const bank_permission = document.getElementById("bank_permission").value.replace(/'/g, "''");
                ...
                {(() => {
                  if (i[0] === "id") {
                    ...
                    if (menuLevel === "1") {
                      return (
                        <>
                          <div className="mb-3 hidden flex items-center">
                            <label className="w-[30%] hidden capitalize text-sm font-semibold text-gray-400">
                              {i[0].replace(/_/g, " ")}
                            </label>
                            <select
