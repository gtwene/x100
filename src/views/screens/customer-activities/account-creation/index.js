...
useEffect(()=>{
    axios.post(API_SERVER + '/get-customer-category' , {csType : accountTypes}).then((res)=>{
  ...
  useEffect(() => {
    ...
    const getCustomerCategory = () => {
        axios.post(API_SERVER + "/get-code-details", { 
        code: "CUT",
    ...
    async function getIntroductorySource() {
        axios.get(API_SERVER + "/get-introductory-source").then(function (response) {
  ...
  getSalutation?.map((i) => (
    ...
    const handleOnChange = {
        ...
        customerSegment: async function(value) {
            const data = await  axios.post(API_SERVER + "/get-sub-segment-cat" , {segmentCode : value})
        ...
        sector: async function(value) {
            ...
            const data = await  axios.post(API_SERVER + "/get-sub-sector" , {sectorClassCode : value})
            if(data.data.length > 0){
                const response = data.data
                const arr = []
